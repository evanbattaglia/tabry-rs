...
pub fn split_with_comppoint(compline: &str, comppoint: usize) -> Result<TokenizedResult, shell_words::ParseError> {
...
mod tests {
    ...
    fn assert_result(compline: &str, comppoint: usize, expected_cmd_basename: &str, expected_arguments: Vec<&str>, expected_last_argument: &str) {
          let res = split_with_comppoint(compline, comppoint).unwrap();
          let expected_args_vec = expected_arguments.iter()
    ...
    fn test_tokenizer_returns_command_basename_argument_last_argument() {
        assert_result(
            "foo/bar abc def ghi", 13, // input
            "bar", vec!["abc"], "def", // expected
        );
    }
    ...
    fn test_tokenizer_handles_quotes_and_backslashes_like_a_shell() {
      assert_result(
          r#""/foo bar/waz" a'b 'c\ d "ef g" "hi jk" lmn"#, 38,
          "waz", vec!["ab c d", "ef g"], "hi jk"
      );
    }
